...
# Global FastMCP server - will be initialized in initialize_server()
...
def _register_tools() -> None:
    ...
    # Apply decorators and register tools
    app.tool()(mcp_access_log("list_datasources")(tool_error_handler(list_datasources)))
    app.tool()(mcp_access_log("list_metrics")(tool_error_handler(list_metrics)))
    app.tool()(
        mcp_access_log("get_metric_metadata")(tool_error_handler(get_metric_metadata))
    )
    app.tool()(mcp_access_log("query_instant")(tool_error_handler(query_instant)))
    app.tool()(mcp_access_log("query_range")(tool_error_handler(query_range)))
    app.tool()(
        mcp_access_log("get_metric_labels")(tool_error_handler(get_metric_labels))
    )
    app.tool()(mcp_access_log("get_label_values")(tool_error_handler(get_label_values)))
    app.tool()(
        mcp_access_log("find_metrics_by_pattern")(
            tool_error_handler(find_metrics_by_pattern)
        )
    )


def get_app() -> FastMCP:
    """Get the FastMCP app instance, initializing if needed for tests."""
    global app
    if app is None:
        # For tests - initialize with minimal config
        initialize_server()
    assert app is not None, "Failed to initialize FastMCP app"
    return app


def initialize_server() -> None:
    """Initialize the server with configuration."""
    global config_loader, app
